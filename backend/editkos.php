...
if (!$id || !$namakos || !$alamatkos || !$hargasewa || !$tipe || !$fasilitas || !$date) {
...
try {
    if (isset($_FILES['url_image']) && $_FILES['url_image']['error'] === UPLOAD_ERR_OK) {
        ...
        $statement = $database_connection->prepare("UPDATE `tambah_kost` SET `namakos`=?, `alamatkos`=?, `hargasewa`=?, `tipe`=?, `fasilitas`=?, `img`=?, `date`=? WHERE `id`=?");
        $statement->execute([$namakos, $alamatkos, $hargasewa, $tipe, $fasilitas, $upload_directory . $namafile, $date, $id]);
    } else {
        $statement = $database_connection->prepare("UPDATE `tambah_kost` SET `namakos`=?, `alamatkos`=?, `hargasewa`=?, `tipe`=?, `fasilitas`=?, `date`=? WHERE `id`=?");
        $statement->execute([$namakos, $alamatkos, $hargasewa, $tipe, $fasilitas, $date, $id]);
    }
